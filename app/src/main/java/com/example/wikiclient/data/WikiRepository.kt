...
interface WikiRepository {
    suspend fun getArticle(keyword: String, limit: Int): GetArticlesResponse?
}
...
class WikiRepositoryImpl : WikiRepository {
    ...
    private val wikiSevice = retrofit.create(WikiApiClient :: class.java)

    // Web APIからデータを取得するメソッド
    override suspend fun getArticle(keyword: String, limit: Int): GetArticlesResponse? {
        return wikiSevice.getArticleList(keyword, limit).body()
    }
}
